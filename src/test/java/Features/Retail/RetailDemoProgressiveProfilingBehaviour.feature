@US07 @retail-demo
Feature: Retail_Demo_US07 Progressive Profiling Behaviour Test
  ...
  @retail-progressive-profiling @retail-demo
  Scenario Outline: US07-01 Retail application should complete the progressive profile behaviour of any valid customer
    ...
    Then User provides "<Email>" to join for the loyalty program of the Retail App for Progressive Profiling
    And User should open Mailinator site and provide "<Email>", and then authorize the subscription for Progressive Profiling
    Given User provides access token to Deloitte security page
    And User provides valid email signup details like "<Email>", "<T&C>", "<ReceiveEmails>" for Progressive Profiling
    ...
    Then User feeds the test data of Retail demo "<Email>", "<T&C>", "<ReceiveEmails>" to csv file
    Then User clicks on profile icon on welcome screen
    Given Go to Account Details and Update the "<FirstName>", "<LastName>", "<Mobile>", "<Address>", "<Country>", "<State>", "<City>", "<ZipCode>", "<Month>", "<Day>" details and submit the changes
...
#    New Steps are observed on 6:46PM 13-10-2022, Deloitte Security Access Page is appearing
#    Given User provides access token to Deloitte security page
    Given User verifies the accessibility to Retail App

    And User clicks on login button of Retail welcome screen
    And User provides username as "<Email>" and password to Retail Demo App - previous
    ...
    Examples:
      ...
      | automationciamsgodemoprogressivepro@mailinator.com |True|False|positive| null|   Robinson |Padilla|9090909090|Eastern Palace Phase 1|United States|California|Los Angeles|90016|09|09|
#      | automationciamsgoprogressive@mailinator.com |True|False|positive| null|   Robinson |Padilla|9090909090|Eastern Palace Phase 1|United States|California|Los Angeles|90016|09|09|