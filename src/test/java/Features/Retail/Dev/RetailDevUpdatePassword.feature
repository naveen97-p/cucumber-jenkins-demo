@US05 @retail-dev
Feature: Retail_Dev_US05 Update User Password Scenario
...
#    Then User Accepts Cookies for Retail App
  ...
  @retail-update-password-dev @retail-dev
  Scenario Outline: US05-01 Verify Retail user is able to update password, then login with updated password and verify user shouldn't able to login with old password
    ...
    And User provides valid email signup details like "<Username>", "<FirstName>", "<LastName>","<T&C>","<ReceiveEmails>" update password Dev
    ...
    Then User feeds the test data of Retail "<Username>", "<FirstName>", "<LastName>","<T&C>","<ReceiveEmails>" to csv file dev update password
    ...
    And User provides username as "<Username>" and password to Retail Dev App update password - previous
    And User Should able to login with "<TypeOfTesting>" approach and should land on Retail home page and pass firstname "<FirstName>"
   ...
   #Test--User tries to login with Updated password
    And User opens the given URL "RETAIL_DEV"
    And User clicks on login button of Retail welcome screen
    And User provides username as "<Username>" and new password to Retail Dev App update password
    And User Should able to login with "<TypeOfTesting>" approach and should land on Retail home page and pass firstname "<FirstName>"
    And User should logout from Retail app
   #Test--User tries to login with old password and site should not allow
    And User opens the given URL "RETAIL_DEV"
    And User clicks on login button of Retail welcome screen
    And User provides username as "<Username>" and old password to Retail Dev App update password
    ...
    Examples:
      ...
      | retaildevupdatepassword@mailinator.com | positive      | automation |automation|True|False  |
...
#    And User opens the given URL "RETAIL_DEV"
#    And User clicks on login button of Retail welcome screen
#    And User provides username as "<Username>" and new password to Retail App
#    And User Should able to login with "<TypeOfTesting>" approach and should land on Retail home page and pass firstname "<Firstname>"
#    And User should logout from Retail app
#   #Test--User tries to login with old password and site should not allow
#    And User opens the given URL "RETAIL_DEV"
#    And User clicks on login button of Retail welcome screen
#    And User provides username as "<Username>" and old password to Retail App
#    And User Should able to login with "negative" approach and should not land on Retail home page
#    Then User terminates the browser
#    Examples:
#      | Username                 | TypeOfTesting | Firstname  |
#     | automationdevuser10@mailinator.com | positive      | automation |
